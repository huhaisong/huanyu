...
public class TitleItemDecoration extends RecyclerView.ItemDecoration {
    ...
    private List<LiuHeIndexItem> mData;
    ...
    public TitleItemDecoration(Context context, List<LiuHeIndexItem> data) {
        super();
        mData = data;
        for (LiuHeIndexItem bankModel :
                mData) {
            if ("#".equals(bankModel.getKey())) {
                bankModel.setKey("热门推荐");
            }
        }
        mPaint = new Paint();
        mBounds = new Rect();
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if (position > -1) {
                if (position == 0) {//等于0的时候绘制title
                    ...
                    if (null != mData.get(position).getKey() && !mData.get(position)
                            .getKey().equals(mData.get(position - 1).getKey())) {
    ...
    private void drawTitle(Canvas c, int left, int right, View child, RecyclerView.LayoutParams params, int position) {
        ...
        mPaint.getTextBounds("   "+mData.get(position).getKey(), 0, ("   "+mData.get(position).getKey()).length(), mBounds);
        c.drawText("   "+mData.get(position).getKey(),
    ...
    public void getItemOffsets(Rect outRect, View view, RecyclerView parent, RecyclerView.State state) {
        ...
        if (position > -1) {
            ...
            if (position == 0) {
                ...
                if (mData.size() <= position) return;
                if (null != mData.get(position).getKey() &&
                        !mData.get(position).getKey().equals(mData.get(position - 1).getKey())) {
