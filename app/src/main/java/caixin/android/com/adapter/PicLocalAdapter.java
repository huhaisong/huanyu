...
import caixin.android.com.utils.ImageLoaderUtils;
import caixin.android.com.utils.OnItemLongClickListener;
 ...
 */
public class PicLocalAdapter extends
        RecyclerView.Adapter<PicLocalAdapter.ViewHolder> {
    ...
    public PicLocalAdapter(Context context, onAddPicClickListener mOnAddPicClickListener) {
    ...
    public ViewHolder onCreateViewHolder(ViewGroup viewGroup, int i) {
        View view = mInflater.inflate(R.layout.local_pic_video_layout,null);
    ...
    public void onBindViewHolder(final ViewHolder viewHolder, final int position) {
        if (selectMax == 1) {
            viewHolder.mImg.setImageResource(R.drawable.icon_add_video);
        ...
        if (getItemViewType(position) == TYPE_CAMERA) {
            viewHolder.mImg.setOnClickListener(v -> mOnAddPicClickListener.onAddPicClick(position));
            ...
            if (chooseModel == PictureMimeType.ofAudio()) {
                ...
                ImageLoaderUtils.loadCropImage(viewHolder.mImg,PictureMimeType.isContent(path) && !media.isCut() && !media.isCompressed() ? String.valueOf(Uri.parse(path))
                        : path);
            }
            //itemView 的点击事件
