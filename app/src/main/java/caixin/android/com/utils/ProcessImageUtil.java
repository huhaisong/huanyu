...
import caixin.android.com.widget.ImageResultCallback;
...
public class ProcessImageUtil extends ProcessResultUtil {
    ...
    public ProcessImageUtil(FragmentActivity activity) {
        ...
        mCameraResultCallback = new ActivityResultCallback() {
            ...
            public void onSuccess(Intent intent) {
                if (mNeedCrop) {
                    ...
                    if (Build.VERSION.SDK_INT >= 24) {
                        uri =   FileProvider.getUriForFile(mContext, Application.getInstance().getPackageName()+ ".fileprovider", mCameraResult);
                    } else {
            ...
            public void onFailure() {
                ToastUtils.show(R.string.img_camera_cancel);
            }
        };
        mAlumbResultCallback = new ActivityResultCallback() {
            ...
            @Override
            public void onFailure() {
                ToastUtils.show(R.string.img_alumb_cancel);
            }
        };
        mCropResultCallback = new ActivityResultCallback() {
            ...
            @Override
            public void onFailure() {
                ToastUtils.show(R.string.img_crop_cancel);
            }
        };
    }
    ...
    private File getNewFile() {
        // 裁剪头像的绝对路径
        File dir = new File(Contact.CAMERA_IMAGE_PATH);
        if (!dir.exists()) {
            dir.mkdirs();
        }
    ...
    private void crop(Uri inputUri) {
        ...
        try {
            ...
            try {
                Uri resultUri  = FileProvider.getUriForFile(mContext, Application.getInstance().getPackageName()+ ".fileprovider", mCorpResult);
                if (resultUri == null || mFragment == null || mContext == null) {
                    return;
                }
                UCrop uCrop = UCrop.of(inputUri, resultUri)
